...
<br>
  ...
  if(isset($_POST['search_submit'])){
    $search = $_POST['med_name'];

    $sql = "SELECT * FROM item where name LIKE'%{$search}%' or brand LIKE '%{$search}%' or description LIKE '%{$search}%'";
...
<tr>
          ...
          <td>
              <a type='button' class='edit_button' href='med_edit.php?med_Id=<?php echo $row['med_Id']?>'>Edit</a>
              <a type='button' class='del_button' href='del_med.php?med_Id= <?php echo$row['med_Id']?>'>Delete</a>
          </td>
          <td><button type='button' class='button'> <a href='med_profile.php?med_Id=<?php echo $row['med_Id']?>'>Details</a></button></td>
      </tr>
  ...
  $sql = "SELECT * FROM item WHERE category='Medicine'";
  $result = mysqli_query($conn, $sql);
// Check if any row is returned
