                    ...
                    if($size_check !== false){
                        if($_FILES["myfile"]["size"] < 50000000){
                            if($img_type == 'jpg' || $img_type == 'jpeg' || $img_type == 'png'){
                                ...
                                if($output){
                                $query = "INSERT INTO item(name, brand, category, sub_category, price, per, description, image, reorder, quantity)VALUES('$name','$brand','Other','$category','$price','$per','$description','$path','$reorder', '$quantity')";
